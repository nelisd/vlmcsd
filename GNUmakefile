...
.DEFAULT:
	...
	+@$(MAKE) -j$(MAX_THREADS) -C src $@ FROM_PARENT=1 PROGRAM_NAME=$(PROGRAM_NAME) CLIENT_NAME=$(CLIENT_NAME) MULTI_NAME=$(MULTI_NAME) DLL_NAME=$(DLL_NAME) A_NAME=$(A_NAME) ELF=$(ELF) UNIX=$(UNIX) WIN=$(WIN) PE=$(PE) CYGWIN=$(CYGWIN) ANDROID=$(ANDROID) DARWIN=$(DARWIN) MINIX=$(MINIX) MINGW=$(MINGW) BSD=$(BSD) FREEBSD=$(FREEBSD) NETBSD=$(NETBSD) OPENBSD=$(OPENBSD) THREADS=$(THREADS) SOLARIS=$(SOLARIS) LINUX=$(LINUX) HURD=$(HURD)
